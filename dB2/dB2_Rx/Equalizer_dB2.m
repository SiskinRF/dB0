function [Equalized, Equalized_PCA, RsCinrdB, ChanResponseGrid, ChanResponseGrid_PCA] = ...
         Equalizer_dB2(TxGrid, RxGrid, UsePCA)

% 1. Definitions
%--------------------------------------------------------------------------

EqualizerGrid        = ones(72, 14);
ChanResponseGrid     = ones(72, 14);
ChanResponseGrid_PCA = ones(72, 14);
Equalized            = ones(72, 28);
Equalized_PCA        = ones(72, 28);
RsCinrdB             = zeros(14, 1); % SNR estimate at each reference symbol
...
if (UsePCA == 1) % Refine the estimate using principle component analysis
  ...
  GridSymbolNum = 1;
  for SymbolNum = 2:2:28
    [Channel_Estimate_PCA, SnrEst]  = ComputeEigenDecomposition_dB2(ChanResponseGrid(1:72, GridSymbolNum), 1);
