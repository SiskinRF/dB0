function [TxGrid, RefSignals, TBLK, TBLK_CNTL] = ConstructResourceGrid_dB4()

% 1. Get resource element values and map to BPSK symbols
%--------------------------------------------------------------------------

% 1a. Get cntl message to Tx and get original message for comparision.
[SCWRD_CNTL, TBLK_CNTL] = GetMessageCntl_dB4();

% 1b. Get data messages to Tx and get original message for comparison.
[SCWRD, TBLK] = GetMessageData_dB4();
SCWRD_DATA    = reshape(SCWRD, 1, []);

% 1c. Get scrambled reference signals
RefSignals = GetRefSignals_dB4();

% 1d. Get unused signals - available for future use
UNUSED = GetMessageUnused_dB4();

% 1e. Map bits to BPSK or QPSK symbols
RefSymbols  = MapperQAM_dB4(RefSignals, 2);
CntlSymbols = MapperQAM_dB4(SCWRD_CNTL, 2);
DatSymbols  = MapperQAM_dB4(SCWRD_DATA', 4);
UnsdSymbols = MapperQAM_dB4(UNUSED', 2);
