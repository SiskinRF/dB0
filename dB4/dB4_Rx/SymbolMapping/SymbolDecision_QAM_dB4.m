function [output, evm] = SymbolDecision_QAM_dB4(input)
...
% 2. Basic BPSK or QPSK demapping (for data) - hard decision
%--------------------------------------------------------------------------
output = zeros(600, 80);

% 2a. Demap control, and unused elements
...
for SymbolNum = 2:2:80
  for ElementNum = 1:600
    ...
    if (ElementType == ResourceElement_Cntl)
      if (real(input(ElementNum, SymbolNum)) < -0.01)
        output(ElementNum, SymbolNum) = -1 + 0j;
      elseif (real(input(ElementNum, SymbolNum)) > 0.01)
        output(ElementNum, SymbolNum) = 1 + 0j;
      else
        output(ElementNum, SymbolNum) = 0 + 0j;
      end
    ...
    elseif (ElementType == ResourceElement_Dat)
      if (real(input(ElementNum, SymbolNum)) < -0.01 && imag(input(ElementNum, SymbolNum)) < -0.01)
        output(ElementNum, SymbolNum) = -0.7071 - 0.7071j;
      elseif (real(input(ElementNum, SymbolNum)) < -0.01 && imag(input(ElementNum, SymbolNum)) > 0.01)
        output(ElementNum, SymbolNum) = -0.7071 + 0.7071j;
      elseif (real(input(ElementNum, SymbolNum)) > 0.01 && imag(input(ElementNum, SymbolNum)) > 0.01)
        output(ElementNum, SymbolNum) = 0.7071 + 0.7071j;
      elseif (real(input(ElementNum, SymbolNum)) > 0.01 && imag(input(ElementNum, SymbolNum)) < -0.01)
