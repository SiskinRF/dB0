function [RxTBLK_CNTL, RxTBLK, CNTL_CRC, RxCRC] = DeConstructResourceGrid_dB4(RxGrid) 
...
CntlBits = DemapperBPSK_dB4(CntlSignals);

% 3b. Unscramble the Cntl bits
id      = 103;
RxSCWRD = Descrambler_dB4(id, CntlBits);
...
DatBits  = DemapperQPSK_dB4(DatSignals);

% 4b. Parse out the two scrambled code words and descramble
RxSCWRD2 = zeros(600, 78);
RxCWRD2  = zeros(600, 78);
...
for SymbolNum = 1:78
  id      = 103;
  RxCWRD2(:, SymbolNum) = Descrambler_dB4(id, RxSCWRD2(:, SymbolNum));
end

% 4c. Average repetitions
RxCBLK_INV2 = zeros(120, 78);
