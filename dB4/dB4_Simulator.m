...
Settings.SNR_dB                = 0;          % Thermal Noise
...
Time_Advance   = -15;  % Time in samples into cyclic prefix (left) from start of symbol
...
Preamble = ConstructPreamble_dB4(Settings.SampleRate);

% 4b. Construct resource grid with test signals
[Tx_ResourceGrid, RefSignals, TBLK, TBLK_CNTL] = ConstructResourceGrid_dB4();

% 4c. Modulate the packet to transmit
[Payload, Tx_evm] = OfdmModulator_dB4(Tx_ResourceGrid, Settings.SampleRate);
...
% 6b. Packet Detection
PacketDetFlag = Packet_Detector_dB4(Packet_Dec);
...
if (CorrectOffset == 1)
  [FreqOffset_Course, FreqOffset_Fine] = Detect_FrequencyOffset_dB4(PacketDetFlag, Packet_Dec);
...
PeakPosition = Detect_TimingOffset_dB4(Packet_Dec);
Payload_Dec  = Packet_Dec(PeakPosition+1:end);
 
% 6e. Demodulation
Rx_ResourceGrid = OfdmDemodulator_dB4(Payload_Dec, Time_Advance);

% 6f. Equalization
[Rx_Signal_Eq, Rx_Signal_Eq_Eigen, RsCinrdB, h_chan, h_chan_Eigen] = ...
Equalizer_dB4(Tx_ResourceGrid, Rx_ResourceGrid, PCAflag);
...
if (LLR_Correction == 0)
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_QAM_dB4(Rx_Signal_Eq);
else
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_BPSK_LLR_dB1(Rx_Signal_Eq, h_chan);
end

% 6h. Pull Rx Msg from Rx_Signal_Eq
[RxTBLK_CNTL, RxTBLK, CNTL_CRC, RxCRC] = DeConstructResourceGrid_dB4(Rx_Signal_Eq); 
