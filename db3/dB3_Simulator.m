...
Settings.FrequencyOffset       = 25000;
...
Preamble = ConstructPreamble_dB3(Settings.SampleRate);
...
[Payload, Tx_evm] = OfdmModulator_dB3(Tx_ResourceGrid, Settings.SampleRate);
...
if(Packet_Source == 0)
  ...
  PacketStruct = load("PlutoData_dB3.mat","PacketPlutoTrim");

  figure(1);
  plot(1:length(PacketStruct.PacketPlutoTrim), real(PacketStruct.PacketPlutoTrim(1:end))); hold on;
  plot(1:length(PacketStruct.PacketPlutoTrim), imag(PacketStruct.PacketPlutoTrim(1:end)));
  title('Tx Packet');
  title('Packet - SDR Captured');

  Packet = (1/512)*PacketStruct.PacketPlutoTrim';
...
% 6b. Packet Detection
PacketDetFlag = Packet_Detector_dB3(Packet_Dec);
...
if (CorrectOffset == 1)
  [FreqOffset_Course, FreqOffset_Fine] = Detect_FrequencyOffset_dB3(PacketDetFlag, Packet_Dec);
...
PeakPosition = Detect_TimingOffset_dB3(Packet_Dec);
Payload_Dec  = Packet_Dec(PeakPosition+1:end);
 
% 6e. Demodulation
Rx_ResourceGrid = OfdmDemodulator_dB3(Payload_Dec, Time_Advance);

% 6f. Equalization
[Rx_Signal_Eq, Rx_Signal_Eq_Eigen, RsCinrdB, h_chan, h_chan_Eigen] = ...
Equalizer_dB3(Tx_ResourceGrid, Rx_ResourceGrid, PCAflag);
...
if (LLR_Correction == 0)
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_BPSK_dB3(Rx_Signal_Eq);
else
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_BPSK_LLR_dB3(Rx_Signal_Eq, h_chan);
end

% 6h. Pull Rx Msg from Rx_Signal_Eq
[RxTBLK_CNTL, RxTBLK, RxCRC] = DeConstructResourceGrid_dB3(Rx_Signal_Eq); 
