...
function PreambleB = GeneratePreambleB_dB1(SampleRate)
  
  assert(SampleRate == 15.36e6 || SampleRate == 30.72e6, "Invalid SampleRate");

  % Preamble B lasts 1152 or 2304 samples (75usec)
  if (SampleRate == 15.36e6)
    NumberOfSamplesToRetain = 1024;
  else
    NumberOfSamplesToRetain = 2048;
  end
  
  % Define Zadoff-Chu Sequence
  Nzc         = 887;           % Waveform bandwidth is less than that of AGC
  ...
  if(SampleRate == 15.36e6)
    CP = PreambleB(1, 897:1024);
  else
    CP = PreambleB(1, 1793:2048);
  end
