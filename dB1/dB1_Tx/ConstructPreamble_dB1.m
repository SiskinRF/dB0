function Preamble = ConstructPreamble_dB1(SampleRate)

  % Scalings determined by eyeballing the packet from Fig1
  ScalingAGC = 1.1;
  ScalingA   = 0.8;
  
  AgcBurst  = GenerateAgcBurst_dB1(SampleRate) * ScalingAGC;
  PreambleA = GeneratePreambleA_dB1(SampleRate) * ScalingA;
  PreambleB = GeneratePreambleB_dB1(SampleRate);
