...
Settings.SampleRate       = 30.72e6;     % (Tx uses 2048 point FFT)
...
Settings.SNR_dB                = 30;          % Thermal Noise
...
Time_Advance   = -25;  % Time in samples into cyclic prefix (left) from start of symbol
PCAflag        = 1;  % 0 = No / 1 = Use Principle Component Analysis in Equalizer
...
Preamble = ConstructPreamble_dB1(Settings.SampleRate);

% 4b. Construct resource grid with test signals
[Tx_ResourceGrid, RefSignals, TBLK, TBLK_CNTL] = ConstructResourceGrid_dB1();

% 4c. Modulate the packet to transmit
[Payload, Tx_evm] = OfdmModulator_dB1(Tx_ResourceGrid, Settings.SampleRate);
...
% 6b. Packet Detection
PacketDetFlag = Packet_Detector_dB1(Packet_Dec);
...
%title('Rx Packet - Decimated 15.36MHZ');

% 6c. Frequency Offset Determination and Correction
if (CorrectOffset == 1)
  [FreqOffset_Course, FreqOffset_Fine] = Detect_FrequencyOffset_dB1(PacketDetFlag, Packet_Dec);

  n = 1:length(Packet_Dec); 
  NCO_Signal = exp(-1i*2*pi*n*FreqOffset_Course/15.36e6);
  Packet_Dec = Packet_Dec .* NCO_Signal;

  NCO_Signal = exp(-1i*2*pi*n*FreqOffset_Fine/15.36e6);
  Packet_Dec = Packet_Dec .* NCO_Signal;
end

% 6d. Timing Offset Determination and Correction
PeakPosition = Detect_TimingOffset_dB1(Packet_Dec);
Payload_Dec  = Packet_Dec(PeakPosition+1:end);
 
% 6e. Demodulation
Rx_ResourceGrid = OfdmDemodulator_dB1(Payload_Dec, Time_Advance);

% 6f. Equalization
[Rx_Signal_Eq, Rx_Signal_Eq_Eigen, RsCinrdB, h_chan, h_chan_Eigen] = ...
Equalizer_dB1(Tx_ResourceGrid, Rx_ResourceGrid, PCAflag);
...
if (LLR_Correction == 0)
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_BPSK_dB1(Rx_Signal_Eq);
else
  [Rx_Signal_Eq, Rx_evm] = SymbolDecision_BPSK_LLR_dB1(Rx_Signal_Eq, h_chan);
end

% 6h. Pull Rx Msg from Rx_Signal_Eq
[RxTBLK_CNTL, RxTBLK, CNTL_CRC, RxCRC] = DeConstructResourceGrid_dB1(Rx_Signal_Eq); 
...
for SymbolNum = 1:39
  ...
  if (isequal(TBLK(:, SymbolNum), RxTBLK(:, SymbolNum)))
    disp(['Success! Tx and Rx for ' num2str(SymbolNum) ' are the same!']);
  else
    disp(['Error! Tx and Rx for ' num2str(SymbolNum) ' do not match']);
  end
